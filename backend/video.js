...
class Grab {
  async getVideo(uri) {
    try {
      ...
      } else if (uri.includes("gofilmes.me/play/tf.php")) {
  ...
  async extractGofilmesVideo(uri) {
    try {
      ...
      // Selecionar o quarto <script>
      const scriptContent = $('script').eq(3).html();
      ...
      const videoUrlMatch = scriptContent.match(/sources:\s*\[\{'file':'(https?:\/\/[^']+)'/);
      
      if (videoUrlMatch && videoUrlMatch[1]) {
        return { source: videoUrlMatch[1] };
      }
